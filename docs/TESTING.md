...
| --- | --- | --- |
| Deterministic cultivation rules, assets, and package manifest | pytest + branch coverage, minimum 99% | 74 tests; 100% statements and branches (456 statements / 212 branches) |
| Godot RPG rules and scenes | headless GDScript harness | 1,290 assertions passing, including save-v13 ordered enemy intelligence and v1–v12 migration, exact four-enemy intent/counter windows, three non-overlapping spoor investigations, knowledge-independent battle results, zero-HP restore rejection, neutral unknown-boss disclosure, crash-consistent primary/temporary/backup recovery, stale-branch replacement, four-artifact anti-downgrade barriers, safe new-game confirmation, accessibility settings, bounded side stories, modal no-spoiler journal pages, persistent discoveries, companion footprints, transitions, occlusion, harvesting, enemy atlases, deterministic non-authoritative map details, boss resolution, and exploration contracts |
| Godot RPG chapter path | independent headless E2E | 149 checks: protected new game → resumable side stories and journal → persistent discoveries → unknown-intent disclosure → three studied spoors and `灵物志 3/3` → non-default enemy save/restore → exact warden counter window → ending and route-aware epilogue → completed-save resume → replay clearing intelligence → 0/3 bypass ending |
| Godot keyboard/controller/mouse path | physical events + focus assertions | 78 checks: genuine mouse TabBar selection → Q/RB journal paging → large-text PageDown/D-pad scrolling and focus exit → J/Y modal open/close and movement blocking → keyboard E and controller A spoor interaction → title/dialogue/portrait focus safety → side-story choices → movement/companion trail → pause/accessibility round-trip → cross-map return → battle focus |
| Godot performance and lifecycle | versioned fixed-clock headless budget | 100,000 collision moves, 50,000 bounded companion-trail updates, and 2,000 complete battle-rule loops each under 2.5 s; 20 fixed-60-FPS scene create/destroy cycles under 5 s; static scene remains 110 nodes, measured title/path/dialogue/battle-immediate/battle-stable/journal peaks are 118/116/120/120/120/120, detail rebuilds remain map-bounded, and root-child leaks remain zero |
| Godot RPG package and reference captures | double export/capture | byte-reproducible PCK and manifest with SHA-256/size/source state verified; nine runtime resources present and nine `tests/`/`tools/` resources absent; consecutive 32-file PNG capture runs have identical SHA-256 sets, including the detailed ferry/path, enemy-spoor, and scrollable-intel states |
