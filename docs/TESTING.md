...
| Godot performance and lifecycle | versioned fixed-clock headless budget | 100,000 collision moves, 100,000 patrol advances, 50,000 bounded companion-trail updates, and 2,000 complete battle-rule loops each under 2.5 s; 20 fixed-60-FPS scene create/destroy cycles sampling 13 states under 7 s with all three reveal modes and automatic rendering disabled; static scene is 114 nodes, measured peak is 124, patrol state is 122, worksite dialogue is 124, each spring state is 115, completion is 117, camera/world invariants and map-bounded detail rebuilds hold, and root-child leaks remain zero |
| Godot RPG package and reference captures | double export/capture | eight source atlases regenerate twice and match their Git-index blobs byte-for-byte; byte-reproducible 681,960-byte PCK with SHA-256 `7fbbf9d9d6c634cc7643fc0b9c7b54539e9af5e55bf97ce6f3a114d230aee888` explicitly probes 22 required runtime resources, including the settings store, main presentation controller, enemy semantic adapter/catalog, and camera policy, while excluding nine `tests/`/`tools/` resources; two consecutive 40-PNG capture runs have identical aggregate SHA-256 `ce846a88dc3ac3f841f883e5872b3511472a0c69c7a500df2fe2f23763abdf46`, including readable standard/instant title-pause controls, expanded ferry/path framing, semantic battle poses, ferry-runner map/choice/worksite states, all three life landmarks, and all three spring ritual states |
