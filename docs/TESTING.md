...
| Godot RPG rules and scenes | headless GDScript harness | 614 assertions passing, including resumable/dynamic chapter epilogue, modal journal/no-spoiler content, stable paper-portrait switching, three persistent discoveries/save v10, bounded companion footprints, focus-blocking/reduced-motion transitions, Y-depth occlusion, selectable harvesting, enemy-atlas selection, presentation independence, shared-resolver boss, dialogue, and exploration contracts |
| Godot RPG chapter path | independent headless E2E | 82 checks: dynamic epilogue interruption/restore/two responses → saved journal restoration → companion/protagonist portrait switching → dialogue interruption → persistent ferry/path discoveries → old-rule harvest → mountain/companion restore → enemy/boss restore → ending → replay cutting/regrowth → bypass |
