...
| Godot RPG package and reference captures | warm/fresh export + double capture | eight source atlases regenerate twice and match their Git-index blobs byte-for-byte; with pinned Godot 4.7.1 and export-time text-scene conversion disabled, two normal PCK exports plus two independent clean-cache project copies must all match byte-for-byte; the current four local macOS/arm64 outputs are 709,100 bytes at SHA-256 `e8308c22cda27e45b73fcf35e4fbb37587a266ead18d7be5c277c0864d74d351`; normalized build tuples are provenance rather than canonical-hash keys, so hosted and cross-OS equality require controlled equal-source evidence; the gate probes 22 required runtime resources, including the settings store, main presentation controller, enemy semantic adapter/catalog, and camera policy, while excluding nine `tests/`/`tools/` resources; two consecutive 41-PNG capture runs have identical aggregate SHA-256 `6e1c0f5b3bca174f25726bfec6de61c86bb9962e57641ca29de90717dc68f4b7`, including the 3×2 portrait comparison board, readable standard/instant title-pause controls, expanded ferry/path framing, semantic battle poses, ferry-runner map/choice/worksite states, all three life landmarks, and all three spring ritual states |
