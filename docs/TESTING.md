...
| Godot RPG rules and scenes | headless GDScript harness | 967 assertions passing, including crash-consistent primary/temporary/backup recovery, stale-branch replacement, four-artifact anti-downgrade barriers, post-migration domain validation, safe new-game confirmation for valid/corrupt files, settings-v3 large-text/high-contrast accessibility with migration and persistence, the ferryman and public-basket side stories/save v12, resumable/dynamic dialogue, modal journal/no-spoiler content, five stable paper portraits, three persistent discoveries, bounded companion footprints, focus-blocking/reduced-motion transitions, Y-depth occlusion, selectable harvesting, enemy-atlas selection, presentation independence, shared-resolver boss, and exploration contracts |
| Godot RPG chapter path | independent headless E2E | 126 checks: cancel/confirm new-game protection → ferryman repair/persist/replay-record branches → mountain basket discovery → herbkeeper dialogue interruption/restore → basket return/replay-trail branches → dynamic epilogue interruption/restore/two responses → saved journal restoration → four character portrait IDs → persistent ferry/path discoveries → old-rule harvest → mountain/companion restore → enemy/boss restore → ending → replay cutting/regrowth → bypass |
| Godot keyboard/controller path | physical events + focus assertions | 61 checks: controller cancel/confirm new-game protection → J/Y journal and movement blocking → title/dialogue/portrait focus safety → keyboard ferryman and herbkeeper dialogues + controller choices → keyboard discoveries → movement/companion trail → stable one-button harvest → pause preferences incl. text-size/contrast round-trip → cross-map basket return → battle focus |
...
| Godot RPG package and reference captures | double export/capture | byte-reproducible PCK and manifest with SHA-256/size/source state verified; seven runtime resources present and nine `tests/`/`tools/` resources absent; consecutive 30-file PNG capture runs have identical SHA-256 sets |
