...
| Godot keyboard/controller path | physical events + focus assertions | 42 checks: J/Y journal and movement blocking → title/dialogue/portrait focus safety → keyboard ferryman dialogue + controller choice → keyboard discovery → movement/companion trail → stable one-button harvest → pause preferences incl. text-size/contrast round-trip → two-map interaction → battle focus |
