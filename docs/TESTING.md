...
| --- | --- | --- |
| Deterministic cultivation rules and package manifest | pytest + branch coverage, minimum 99% | 65 tests; 100% statements and branches |
| Godot RPG rules and scenes | headless GDScript harness | 1,156 assertions passing, including save-v13 ordered enemy intelligence and v1–v12 migration, exact four-enemy intent/counter windows, three non-overlapping spoor investigations, knowledge-independent battle results, zero-HP restore rejection, neutral unknown-boss disclosure, crash-consistent primary/temporary/backup recovery, stale-branch replacement, four-artifact anti-downgrade barriers, safe new-game confirmation, accessibility settings, bounded side stories, modal no-spoiler journal pages, persistent discoveries, companion footprints, transitions, occlusion, harvesting, enemy atlases, boss resolution, and exploration contracts |
...
| Godot performance and lifecycle | versioned fixed-clock headless budget | 100,000 collision moves, 50,000 bounded companion-trail updates, and 2,000 complete battle-rule loops each under 2.5 s; 20 fixed-60-FPS scene create/destroy cycles under 5 s; measured title/path/dialogue/battle-immediate/battle-stable/journal peaks are 118/116/120/120/120/120 nodes with zero root-child leaks |
| Godot RPG package and reference captures | double export/capture | byte-reproducible PCK and manifest with SHA-256/size/source state verified; seven runtime resources present and nine `tests/`/`tools/` resources absent; consecutive 32-file PNG capture runs have identical SHA-256 sets, including the enemy-spoor and scrollable-intel states |
