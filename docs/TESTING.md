...
| --- | --- | --- |
| Deterministic cultivation rules and package manifest | pytest + branch coverage, minimum 99% | 61 tests; 100% statements and branches |
| Godot RPG rules and scenes | headless GDScript harness | 588 assertions passing, including modal journal/no-spoiler content, stable paper-portrait switching, three persistent discoveries/save v10, bounded companion footprints, focus-blocking/reduced-motion transitions, Y-depth occlusion, selectable harvesting, enemy-atlas selection, presentation independence, shared-resolver boss, dialogue, and exploration contracts |
| Godot RPG chapter path | independent headless E2E | 71 checks: saved journal restoration → companion/protagonist portrait switching → dialogue interruption → persistent ferry/path discoveries → old-rule harvest → mountain/companion restore → enemy/boss restore → ending → replay cutting/regrowth → bypass |
| Godot keyboard/controller path | physical events + focus assertions | 33 checks: J/Y journal and movement blocking → title/dialogue/portrait focus safety → keyboard discovery → movement/companion trail → stable one-button harvest → pause preferences → two-map interaction → battle focus |
