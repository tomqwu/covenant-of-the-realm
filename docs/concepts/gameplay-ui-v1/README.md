...
结算回声，也不改变 Journey 快照；当前 save v17 不为这些重复观察增加字段。
...
照禾人曾在涨水前转移药苗；调查会写入当前 save v17、改变石面余留并隐藏一次性行动，
...
重新遮住。该偏好只保存在独立 settings 文件；当前 save v17 仍只记录结构化对话行号，
不记录逐字显露进度。
...
ID，没有增加旅程字段，v1–v15 保守迁移，旧运行时以未来版本阻挡。当前 save v17
继续接受这四个稳定 ID。
...
系上纸签。save v11 引入二选一结果，当前 save v17 继续保存，并把它写进札记、章节结算和余波对白，但不发放
资源、战斗加成或隐藏关系数值。
...
暂停、标题和转场都会冻结完整路线。靠近后“问问岑苇”按固定优先级回显挫败、
药篓归圃、药篓留山、已发现未处置药篓、已辨敌迹或默认巡路六种状态之一。全部回声
可重复且不改变 Journey 快照，也不发任务、战斗优势或奖励。save v17 顶层
`path_keeper` 保存位置、相邻目标、方向、停留和礼让状态；v1–v16 从固定起点迁移，
精灵本身不拥有碰撞、任务、战斗、奖励或存档权威。
...
首领替换、不写入 save v17，也不接管鼠标、键盘或手柄输入。截图脚本在写图前冻结
...
敌人目录、回合或伤害规则，不翻译反制行动，也不拥有输入、计时或 save v17 权威。
...
听泉完成后，泉纹沿石室亮起并把当前阶段写入 save v17。每次成功步骤都立即自动
...
回应。逐句中断仍由 save v17 恢复，两个收束选择只留下原创事件回声，不发隐藏奖励。
...
- 岑苇的四点路线、礼让迟滞、六类进度回声与精确恢复来自独立确定性巡山状态；回声不改变 Journey，视觉不拥有碰撞、任务、战斗、奖励或存档权威；
...
- 九种敌足“刚才”势痕均有颜色无关的互异几何与中文结果文字，只从严格匹配的旧势
  ...
  秒与完整/简化动态不改变下一行动、伤害、意图、游戏时序、输入或 save v17；
- 补船木架、晾晒竹架与避雨石棚可由键盘、鼠标和手柄反复查看，不改变 Journey 快照或 save v17；
...
- 静态场景为 117 节点、峰值 127，山道态为 123，陶小满巡路态为 125、端点对话态为
  ...
  札记与普通敌替换的即时/稳定采样均为 126，场景销毁后零泄漏；
- 六套肖像 v2 在同尺寸总览与真实对话框中保留独立轮廓、神情和随身物件，晨桃色只作低频暖光；
- 捕获脚本只在写图时固定动画帧、反馈相位与转场时点；当前清单为 43 张 PNG，新增
  `02-cangquan-enemy-defeat.png`，本轮更新 battle 与 battle-react 两张势痕证据。连续
  两轮重建逐字节一致，聚合 SHA-256 均为
