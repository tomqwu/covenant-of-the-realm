...
1152×648 实际游戏画面。主角、砚青、梁叔、蕙婶、四类敌人、渡口与山道地表已经接入项目原创、确定性
...
结算回声，也不改变 Journey 快照或 save v14 结构。
...
照禾人曾在涨水前转移药苗；调查会写入当前 save v14、改变石面余留并隐藏一次性行动，
...
不跳动。五种头像都是项目代码确定性绘制的明亮纸绘首轮占位，不是从概念图裁切的
...
系上纸签。save v11 引入二选一结果，当前 save v14 继续保存，并把它写进札记、章节结算和余波对白，但不发放
资源、战斗加成或隐藏关系数值。
...
听泉完成后，泉纹沿石室亮起并把当前阶段写入 save v14。每次成功步骤都立即自动
...
回应。逐句中断仍由 save v14 恢复，两个收束选择只留下原创事件回声，不发隐藏奖励。
...
- 七张项目图集在两个临时目录重建后逐字节相等并匹配 Git；十一个地标区域均为 RGBA8、透明边界且含可见像素；
- 补船木架、晾晒竹架与避雨石棚可由键盘、鼠标和手柄反复查看，不改变 Journey 快照或 save v14；
- 山道返程山门由固定 5×5 石桥接回道路，出生点、退路和全部交互锚点不再落在水面；
- 静态场景为 110 节点、峰值 120，三种石室仪轨状态均为 111、完成态为 113，场景销毁后零泄漏；
- 捕获脚本只在写图时固定动画帧、反馈相位与转场时点，连续两轮 34 张 PNG 的 SHA-256 集合完全一致；
- 下一步是纸绘头像精修、相机边界、独立巡逻 NPC 导航和更长的原创支线内容。
