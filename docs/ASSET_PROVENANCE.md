...
`rpg/assets/pixel/zhaohe_landmarks.png` is an original 1536×128 RGBA atlas generated by the same
project-authored `rpg/tools/generate_pixel_assets.gd`. Its eight fixed 192×128 regions contain a
celadon tree, three distinct ferry houses, a timber dock, mountain rock, spring cave, and spring
gate. Every silhouette is assembled from project-local pixel rectangles and the recorded bright
...
The atlas drives all previously geometric buildings, trees, docks, rocks, cave mouths, and gates
in the ferry, mountain-path, and battle views. Houses and trees reuse the existing Y-sorted
foreground nodes; other landmarks are fixed map-canvas regions. These sprites own no collision,
navigation, story, or save authority. The adjacent mountain-return bridge is ground data generated
from the existing original tile atlas, not an imported asset.

`asset_contract.json` schema v2 fixes the file name, atlas/frame dimensions, foot anchor, profile
