...
7. side-story-, discovery-, harvest-, battle-status-, enemy-, dialogue-, and map-aware save v11, title/continue, two-step new-game protection, safe backup recovery, chapter replay, and an input-safe no-spoiler journey journal (implemented; v1–v10 migrate explicitly);
8. mouse, keyboard, and controller parity with physical-event/focus acceptance coverage (implemented);
9. original visual direction and opt-in procedural ambience with recorded provenance (reproducible protagonist/companion/ferryman and enemy atlases, animated map actors, and bright painted-paper dialogue portraits implemented; map/final asset refinement remains);
