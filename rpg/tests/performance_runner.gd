...
func _benchmark_scene_lifecycle_sample(budget: Dictionary) -> Dictionary:
	...
	for _cycle in range(cycles):
		...
		var accent_remaining := 0.70
		if _cycle == 0:
			# Unit, E2E, and input suites own exhaustive attack-accent behavior.
			# Keep one public-contract semantic probe per complete lifecycle sample,
			# while every cycle still arms and advances the real feedback lifecycle.
			...
			accent_remaining = float(immediate_attack_accent["remaining"])
		var advanced_attack_accent: bool = battle_map_canvas.advance_battle_feedback(accent_remaining)
		if not advanced_attack_accent:
			failures.append("生命周期每轮公开推进器都必须接受活动势痕的精确剩余时长")
		if _cycle == 0:
