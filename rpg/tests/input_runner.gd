...
func _run() -> void:
	...
	_expect(game.exploration.restore({"map_id": "cangquan_path", "player_x": ExplorationStateScript.PATH_MARKER_POSITION.x, "player_y": ExplorationStateScript.PATH_MARKER_POSITION.y}), "输入验收移动到晴绳旧石标")
	game._render([])
	await _settle()
	var path_mark_button := _find_action_button(game, "查看旧石标")
	_expect(path_mark_button != null, "晴绳支线入口生成可点击的近距行动按钮")
	if path_mark_button != null:
		await _trigger_mouse_click(path_mark_button.get_global_rect().get_center())
	_expect(game.dialogue.active and game.dialogue.dialogue_id == "path_mark_briefing", "真实鼠标开启晴绳结构化对话")
	_expect(game.get_node("%DialogueLabel").text.contains("晴绳松了"), "鼠标入口显示原创晴绳台词")
	var frozen_path_keeper: Dictionary = game.path_keeper.snapshot()
	await create_timer(0.12).timeout
	_expect(game.path_keeper.snapshot() == frozen_path_keeper, "晴绳活动对话冻结完整岑苇巡山时钟")
	game.skip_dialogue_to_response()
	await _settle()
	var path_mark_focus := root.gui_get_focus_owner()
	_expect(path_mark_focus is Button and path_mark_focus.text == "系成低结，照顾负篓行人。", "晴绳回应默认聚焦第一项且不自动代选")
	await _trigger_joy_button(JOY_BUTTON_A)
	_expect(game.journey.path_mark_response == JourneyStateScript.PATH_MARK_LOW_KNOT, "真实手柄 A 提交低结护篓选择")
	_expect(not game.dialogue.active and game.get_node("%EventLabel").text.contains("负篓人"), "手柄选择单次关闭对话并显示公共后果")
	var path_mark_visual: Dictionary = game.get_node("%MapCanvas").path_mark_visual_contract()
	_expect(path_mark_visual["visible"] and path_mark_visual["shape"] == "low_horizontal_knot", "输入选择立即显示颜色无关的低结地图余留")
	_expect(not path_mark_visual["rule_authority"] and not path_mark_visual["input_authority"] and not path_mark_visual["gameplay_timing_authority"], "真实输入路径中的晴绳几何保持规则、输入与时序零权威")
	await _trigger_key(KEY_E)
	_expect(game.get_node("%EventLabel").text.contains("低结仍系得牢"), "真实键盘 E 重查已系旧石标而不重复开启选择")
	_expect(game.journey.path_mark_response == JourneyStateScript.PATH_MARK_LOW_KNOT, "键盘重查不覆盖手柄提交的晴绳结果")
