...
启动后可选择新游戏或继续本机的版本化存档。当前 save v12 同时记录稳定地图标识、
归一化坐标、对话行号、敌人标识、战斗状态、月芽采集方式、环境见闻、守堤与药篓选择；v1–v11 自动迁移，未知地图、无效对话、未知敌人、非法采集、见闻或支线状态不会被静默放进错误场景。游戏会在成功交互、战斗行动和持续
...
降级和覆盖；v1–v11 迁移也必须通过当前规则、地图与对话校验。只要主文件、备份或
...
锁死主线；采集字段自 save v9 起保存，并由当前 v12 在章节结算中回显。键盘/手柄单键交互采用稳定的
...
凝息，强化后续术式或符箓。两种状态自 save v8 起明确记录，并由当前 v12 继续保存；完整 E2E 会在
