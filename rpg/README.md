...
启动后可选择新游戏或继续本机的版本化存档。当前 save v9 同时记录稳定地图标识、
归一化坐标、对话行号、敌人标识、战斗状态与月芽采集方式；v1–v8 自动迁移，未知地图、无效对话、未知敌人或非法采集状态不会被静默放进错误场景。游戏会在成功交互、战斗行动和持续
...
锁死主线；选择进入 save v9，并在章节结算中回显。键盘/手柄单键交互采用稳定的
...
凝息，强化后续术式或符箓。两种状态自 save v8 起明确记录，并由当前 v9 继续保存；完整 E2E 会在
