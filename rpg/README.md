...
启动后可选择新游戏或继续本机的版本化存档。当前 save v10 同时记录稳定地图标识、
归一化坐标、对话行号、敌人标识、战斗状态、月芽采集方式与环境见闻；v1–v9 自动迁移，未知地图、无效对话、未知敌人、非法采集或见闻状态不会被静默放进错误场景。游戏会在成功交互、战斗行动和持续
...
最近邻过滤与整数像素对齐，并通过 `AnimatedSprite2D` 驱动主角和砚青。四类敌人
...
锁死主线；采集字段自 save v9 起保存，并由当前 v10 在章节结算中回显。键盘/手柄单键交互采用稳定的
...
凝息，强化后续术式或符箓。两种状态自 save v8 起明确记录，并由当前 v10 继续保存；完整 E2E 会在
...
因此没有第二套进度字段，也不会为 save v10 增加冗余状态。
