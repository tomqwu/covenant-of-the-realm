...
probes the packed namespace for six runtime resources and nine excluded `tests/`/`tools/` files,
...
启动后可选择新游戏或继续本机的版本化存档。当前 save v11 同时记录稳定地图标识、
归一化坐标、对话行号、敌人标识、战斗状态、月芽采集方式、环境见闻与守堤选择；v1–v10 自动迁移，未知地图、无效对话、未知敌人、非法采集、见闻或支线状态不会被静默放进错误场景。游戏会在成功交互、战斗行动和持续
移动时自动保存；按 `Esc` 或手柄 Start 可暂停、保存并返回标题。损坏或未知版本
的存档不会被静默载入，主文件写入失败时可从安全备份恢复。只要主文件、备份或
...
最近邻过滤与整数像素对齐，并通过 `AnimatedSprite2D` 驱动主角、砚青和守堤人梁叔。四类敌人
...
锁死主线；采集字段自 save v9 起保存，并由当前 v11 在章节结算中回显。键盘/手柄单键交互采用稳定的
...
凝息，强化后续术式或符箓。两种状态自 save v8 起明确记录，并由当前 v11 继续保存；完整 E2E 会在
...
对话正文、主角回应、梁叔台词与最近四句回顾分别使用砚青、行旅者、守堤人和行旅札记四种明亮纸绘
